...
- (void)viewDidLoad
{
    ...
    UILongPressGestureRecognizer *cellLongPress = [[UILongPressGestureRecognizer alloc]
                                               initWithTarget:self
                                               action:@selector(sendMessage:)];
    [_txtPhone addGestureRecognizer:longPress];
    [_txtCell addGestureRecognizer:cellLongPress];
...
- (IBAction)changeEditMode:(id)sender {
    ...
    if (_sgmtEditMode.selectedSegmentIndex == 0){
        ...
        _txtCell.editMode = NO;
        [_txtPhone setEnabled:YES];
        [_txtCell setEnabled:YES];
        [self.changeButton setHidden:YES];
...
- (void)sendMessage:(UILongPressGestureRecognizer*)gesture {
    MFMessageComposeViewController *controller = [[MFMessageComposeViewController alloc] init];
    if([MFMessageComposeViewController canSendText])
    {
        controller.body = @"SMS message here";
        controller.recipients = [NSArray arrayWithObjects:_txtPhone.text, nil];
